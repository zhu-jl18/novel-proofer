...
| 新任务 | UI 解除关联（不删除、不停止任务） | 不删除 | **任务仍在任务列表中**，可通过「加载任务」重新关联查看状态；当任务进入 `paused/done` 后可继续/下载（校对进行中需先「暂停」后才允许新任务） |
...
- 该缓存会在你点击「清理中间文件」后一并删除
- 如果你仅让任务自动完成（且勾选了“完成后删除调试中间文件”），`output/.jobs/{job_id}/` 可能会被自动清理，但输入缓存仍会保留，以便后续重跑全部
